...
const QRColumn: React.FC<QRColumnProps> = ({ record }) => {
...
import type { CheckListProps, CheckListPeriod, PeriodData, PeriodHour } from "../@types/CheckListType";

const periodMap: Record<keyof CheckListPeriod, number> = {
  morning: 0,
  noon: 1,
  evening: 2,
};
...
function WorkDays({ userId, username }: WorkDaysProps) {
  const today = new Date();
  ...
  const [mainData, setData] = useState<CheckListProps | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      fetch(`${useApiHost()}/workpoint/${userId}`)
        ...
        .then((data: CheckListProps) => {
          if (!isMounted) return;
          setData(data);
          const newStatuses = Array(daysInMonth)
            .fill(null)
            .map(() => Array(3).fill(null));

          const total:PeriodHour | null = {
            morning: 0,
            noon: 0,
            evening: 0,
          };
          ...
          if (
            ...
            (Object.keys(data.checklist) as (keyof CheckListPeriod)[]).forEach(
              (period) => {
...
return (
  ...
  <Stack direction="row" spacing={1}>
        ...
        return (
          ...
          >
            ...
            {[0, 1, 2].map((btnIndex) => {
              ...
              if (status && mainData?.checklist) {
                const periodKey = ['morning', 'noon', 'evening'][btnIndex] as keyof CheckListPeriod;
                const periodData = mainData.checklist[periodKey];
                if (periodData) {
                  imgUrl = periodData;
                }
              }
    ...
    <Modal
      ...
      style={{ padding: 20, maxHeight:'60vh', minWidth:'60vw' }}
    >
